...
public class RatingCommand implements CommandExecutor, TabCompleter {
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        ...
        try { int page = (args.length>=2)? Integer.parseInt(args[1]) : 1; sendRanking(sender, Math.max(1,page)); } catch (Throwable tt) { sender.sendMessage("§c랭킹 오류: " + tt); } catch (NumberFormatException ignored){}
    ...
    private void sendRanking(org.bukkit.command.CommandSender s, int page){
        try{
            java.util.List<com.minkang.ultimate.pixelrating.StorageYAML.PlayerProfile> top = plugin.storage().top(page, 10);
            if (top == null) top = java.util.Collections.emptyList();
            s.sendMessage("§6[랭킹] §7페이지 " + page);
            int base = (page-1)*10;
            for (int i=0;i<top.size();i++){
                com.minkang.ultimate.pixelrating.StorageYAML.PlayerProfile pr = top.get(i);
                s.sendMessage(String.format("§e#%d §f%s §7Elo §b%d §7티어 §a%s", base+i+1, pr.name, pr.rating, pr.tier));
            }
        }catch(Throwable t){
            s.sendMessage("§c랭킹을 불러오지 못했습니다: " + t.getClass().getSimpleName());
            t.printStackTrace();
        }
    }
