...
if (sub.equals("경기장")) {
    ...
    private boolean handleArena(CommandSender sender, String[] args) {
        ...
        if (sub.equalsIgnoreCase("목록")) {
            ...
            for (String s : plugin.arenas().list()) { sb.append(s).append(" "); }
        ...
        if (sub.equalsIgnoreCase("정보")) {
            ...
            ArenaManager.Arena info = plugin.arenas().get(args[2]);
            if (info == null) { sender.sendMessage("경기장을 찾을 수 없습니다."); return true; }
            sender.sendMessage("§e이름: §f"+info.name);
            sender.sendMessage("§e상태: §f"+(info.enabled?"활성":"비활성"));
            sender.sendMessage("§eP1: §f"+(info.p1==null?"미설정":info.p1.getWorld().getName()+" "+info.p1.getBlockX()+","+info.p1.getBlockY()+","+info.p1.getBlockZ()));
            sender.sendMessage("§eP2: §f"+(info.p2==null?"미설정":info.p2.getWorld().getName()+" "+info.p2.getBlockX()+","+info.p2.getBlockY()+","+info.p2.getBlockZ()));
            sender.sendMessage("§eStage: §f"+(info.stage==null?"미설정":info.stage.getWorld().getName()+" "+info.stage.getBlockX()+","+info.stage.getBlockY()+","+info.stage.getBlockZ()));
            sender.sendMessage("§eSpec: §f"+(info.spec==null?"미설정":info.spec.getWorld().getName()+" "+info.spec.getBlockX()+","+info.spec.getBlockY()+","+info.spec.getBlockZ()));
            return true;
        }
    ...
    public List<String> onTabComplete(CommandSender sender, Command command, String alias, String[] args) {
        ...
        if (args.length >= 2 && args[0].equalsIgnoreCase("경기장")) {
            ...
            if (args.length == 3 && Arrays.asList("삭제","활성화","위치설정","정보").contains(args[1])) {
                ...
                names.addAll(plugin.arenas().names());
