...
public class RatingManager {
  ...
  public void applyMatch(java.util.UUID w, java.util.UUID l, boolean draw){
    int ew=getElo(w), el=getElo(l); double sw=draw?plugin.getConfig().getDouble("rating.draw-score",0.5):1.0; double sl=draw?plugin.getConfig().getDouble("rating.draw-score",0.5):0.0;
    double exw=1.0/(1.0+Math.pow(10,(el-ew)/400.0)); double exl=1.0-exw;
    int kw=kFor(ew), kl=kFor(el); setElo(w,(int)Math.round(ew+kw*(sw-exw))); setElo(l,(int)Math.round(el+kl*(sl-exl))); }
  private int kFor(int e){ int d=plugin.getConfig().getInt("rating.k-factor.default",32), hi=plugin.getConfig().getInt("rating.k-factor.high-bracket-k",24), lo=plugin.getConfig().getInt("rating.k-factor.low-bracket-k",40), hT=plugin.getConfig().getInt("rating.k-factor.high-threshold",1800), lT=plugin.getConfig().getInt("rating.k-factor.low-threshold",1000); if(e>=hT) return hi; if(e<=lT) return lo; return d; }
