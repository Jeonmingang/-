...
public class UltimatePixelmonRatingPlugin extends JavaPlugin {
  ...
  @Override public void onEnable(){
    instance=this; saveDefaultConfig(); saveResource("rewards.yml", false);
    ...
    try { sessionManager = new SessionManager(this); }
    catch (Throwable t){ getLogger().log(Level.INFO, "[UPR] SessionManager optional: " + t.getClass().getSimpleName()); }
    try { ProtocolHook.safeInit(this); } catch(Throwable t){ getLogger().log(Level.WARNING, "[UPR] ProtocolLib present but hook init failed: " + t.getClass().getName()); }
    ...
    getLogger().info("[UPR] UltimatePixelmonRating enabled (v1.6.10).");
  }
