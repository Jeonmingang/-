...
public class BanEnforcer {
  ...
  public void reload(){ bannedPokemon.clear(); bannedItems.clear(); for (String s : plugin.getConfig().getStringList("bans.pokemon")) bannedPokemon.add(s.toLowerCase()); for (String s : plugin.getConfig().getStringList("bans.items")){ try { bannedItems.add(Material.valueOf(s)); } catch (IllegalArgumentException ignored) {} } }
