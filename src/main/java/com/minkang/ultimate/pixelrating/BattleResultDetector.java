...
public class BattleResultDetector {
    ...
    public BattleResultDetector(UltimatePixelmonRatingPlugin plugin, RatingManager ratings){
        this.plugin = plugin; this.ratings = ratings;
        // English + Korean common messages (Pixelmon & translations)
        winPatterns = new Pattern[]{
            // {p1} has defeated {p2}!
            Pattern.compile("(?<p1>\w{2,16})\s+(?:has )?defeated\s+(?<p2>\w{2,16})", Pattern.CASE_INSENSITIVE),
            ...
            if (m.find()){
                ...
                if (p2 == null){
                    ...
                    if (s != null){
                        ...
                        org.bukkit.entity.Player oppP = Bukkit.getPlayer(opp);
                ...
                if (A != null && B != null){
                    Bukkit.getScheduler().runTask(plugin, () -> {
                        ratings.recordResultNames(A, B, false);
                    });
                }
                return;
            }
